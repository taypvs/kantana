<?php
class EstimationResult extends MY_Controller{
  ...
  public function index(){
    ...
    // Get Item from database for admin
    ...
    $this->load->view('/price/estimation_result.php', $this->_data);
  }
  ...
  public function filterFull(){
    ...
    $result = $this->MResult->getFullInfo($fromdate, $todate, $project_title, $employee_id, $customer_id, $department_id, $agency_id);
    $json = json_encode($result, JSON_PRETTY_PRINT);
    $fetch_data = json_decode($json,true);
    if (is_array($fetch_data) || is_object($fetch_data) || $fetch_data !== null){
      foreach ($fetch_data as $value) {
        $difference = $value['est_total_selling_price'] - $value['est_total_cost'];
        echo '
          <tr>
            <td>'.addZeroToNumber($value['cus_cost_id']).'</td>
            <td>'.$value['project_title'].'</td>
            <td>'.$value['cus_name'].'<br>'.$value['agency_name'].'</td>
            <td>'.$value['emp_name'].'<br>'.$value['dep_name'].'</td>
            <td> ￥'.number_format($value['est_total_selling_price']).'</td>
            <td> ￥'.number_format($value['est_total_cost']).'</td>';
            if($difference < 0){
